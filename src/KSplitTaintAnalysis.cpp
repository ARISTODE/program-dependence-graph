...
bool pdg::KSplitTaintAnalysis::runOnModule(Module &M)
{
  ...
  analyzeRiskyAPICalls();
  riskyAPICollector.printStatistics();
  
  // analyzePrivateStateUpdate();
...
void pdg::KSplitTaintAnalysis::analyzeRiskyAPICalls()
{
...
void pdg::KSplitTaintAnalysis::analyzePrivateStateUpdate()
{
        ...
        for (auto transFuncNode : transFuncNodes)
        {
          ...
          if (auto transFunc = dyn_cast<Function>(val))
          {
            ...
            for (auto storeInst : storeInsts)
            {
              ...
              // TODO: we should also add all the store insts with all pathConds tainted
