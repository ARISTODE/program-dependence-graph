...
void pdg::ProgramGraph::build(Module &M)
{
  ...
  for (auto gv : call_g.getAnalysisGlobalVar())
  {
    auto &global_var = *gv;
  ...
  for (auto F : call_g.getTaintedFuncs())
  {
  ...
  errs() << "finish building Graph Nodes\n";
  _is_build = true;
}

void pdg::ProgramGraph::bindDITypeToNodes()
{
  auto &call_g = PDGCallGraph::getInstance();
  for (auto F : call_g.getTaintedFuncs())
  {
  ...
  for (auto global_var : call_g.getAnalysisGlobalVar())
  {
