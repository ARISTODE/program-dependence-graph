...
llvm::AnalysisKey pdg::BoundaryAnalysis::Key;
...
pdg::BoundaryAnalysis::Result pdg::BoundaryAnalysis::run(Module &M, ModuleAnalysisManager &MAM)
{
...
void pdg::BoundaryAnalysis::computeDriverImportedFuncs(Module &M)
{
  for (auto &F : M)
  {
    if (F.isDeclaration())
      continue;
    // Skip header-defined functions (simplified check)
...
void pdg::BoundaryAnalysis::computeExportedFuncs(Module &M)
{
  ...
  for (auto &global_var : M.globals())
  {
    ...
    if (!gv_lowest_di_type || gv_lowest_di_type->getTag() != dwarf::DW_TAG_structure_type)
      continue;
    auto gv_di_type_name = dbgutils::getSourceLevelTypeName(*gv_lowest_di_type);
    ...
    // In LLVM 19+, global variables have opaque pointer types
    // We need to get the source element type
    if (auto pt = dyn_cast<PointerType>(global_type))
      global_type = global_var.getValueType();
